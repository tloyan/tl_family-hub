...
- Suffixes de role : `.service`, `.resolver`, `.repository`, `.card`, `.form`, `.hooks`, `.schema`, `.store`, `.spec`, `.module`, `.guard`, `.decorator`, `.filter`, `.event`, `.command`, `.handler`, `.query`
...
- Schema SDL (`.graphql`) = contrat API, source de verite
- `graphql-codegen` genere les types TS backend + hooks Apollo Client
- Fichiers : `schema.graphql` (root) + schemas par domaine
...
│   │   │   │   └── filters/
│   │   │   ├── graphql/
│   │   │   │   ├── schema.graphql    # Root schema stitching
│   │   │   │   ├── household.graphql
│   │   │   │   ├── member.graphql
│   │   │   │   ├── ritual.graphql
